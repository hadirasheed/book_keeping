...
import { userOwnsAccount, userOwnsBook } from "@/lib/ownership";

const DENIED = NextResponse.json({ error: "Not found" }, { status: 404 });

// GET /api/bank-accounts?bookId= — list accounts for a book you own.
...
// POST /api/bank-accounts — create an account under a book you own.
  ...
  try {
    ...
    const { data, error } = await getServiceClient()
      ...
      .insert({
        ...
        currency: (body.currency ?? "KWD").trim() || "KWD",
      })
      .select()
      .single();
    if (error) throw error;
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const userId = await getCurrentUserId();
    if (!(await userOwnsAccount(body.id, userId))) return DENIED;
    ...
    if (body.currency !== undefined)
      updates.currency = body.currency.trim() || "KWD";
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const userId = await getCurrentUserId();
    if (!(await userOwnsAccount(id, userId))) return DENIED;
